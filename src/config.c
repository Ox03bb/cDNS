...
int load_config(const char *filename, Config *cfg) {
    ...
    if (general) {
        ...
        if (pn.ok) cfg->general.project_name = strdup(pn.u.s);

        toml_datum_t ver = toml_string_in(general, "version");
        if (ver.ok) cfg->general.version = strdup(ver.u.s);

        toml_datum_t auth = toml_string_in(general, "author");
        if (auth.ok) cfg->general.author = strdup(auth.u.s);

        toml_datum_t desc = toml_string_in(general, "description");
        if (desc.ok) cfg->general.description = strdup(desc.u.s);
    }
    ...
    if (server) {
        ...
        if (host.ok) cfg->server.host = strdup(host.u.s);

        toml_datum_t port = toml_int_in(server, "port");
        if (port.ok) cfg->server.port = (int)port.u.i;

        toml_datum_t proto = toml_string_in(server, "protocol");
        if (proto.ok) cfg->server.protocol = strdup(proto.u.s);

        toml_datum_t log = toml_string_in(server, "log_level");
        if (log.ok) cfg->server.log_level = strdup(log.u.s);

        toml_datum_t ttl = toml_int_in(server, "ttl");
        if (ttl.ok) cfg->server.ttl = (int)ttl.u.i;

        toml_datum_t cache = toml_bool_in(server, "enable_cache");
        if (cache.ok) cfg->server.enable_cache = cache.u.b;
    }
